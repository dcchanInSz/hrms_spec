...
class PositionModel {
   ...
   * 根据部门获取职位
   */
  static async findByDepartment(departmentId: string | number): Promise<any[]> {
    const result: QueryResult = await query(
      `SELECT p.*, d.name as department_name
       FROM positions p
       LEFT JOIN departments d ON p.department_id = d.id
   ...
   * 获取职位层级
   */
  static async findByLevel(level: number): Promise<any[]> {
    const result: QueryResult = await query(
      `SELECT p.*, d.name as department_name
       FROM positions p
       LEFT JOIN departments d ON p.department_id = d.id
   ...
   * 检查职位是否有员工
   */
  static async hasEmployees(id: string | number): Promise<boolean> {
    const result: QueryResult = await query(
   ...
   * 获取职位薪资范围
   */
  static async getSalaryRange(id: string | number): Promise<any> {
    const result: QueryResult = await query(
