...
class DepartmentModel {
   ...
   * 获取直接子部门
   */
  static async findChildren(parentId: string | number): Promise<any[]> {
    const result: QueryResult = await query(
      `SELECT d.*, m.name as manager_name
       FROM departments d
       LEFT JOIN employees m ON d.manager_id = m.id
   ...
   * 检查部门是否有子部门
   */
  static async hasChildren(id: string | number): Promise<boolean> {
    const result: QueryResult = await query(
   ...
   * 获取部门经理
   */
  static async getManager(id: string | number): Promise<any> {
    const result: QueryResult = await query(
