...
class AuditLogModel {
   ...
   * 获取用户操作日志
   */
  static async findByUser(userId: string | number, limit: number = 20): Promise<any[]> {
    const result: QueryResult = await query(
   ...
   * 获取实体变更历史
   */
  static async findByEntity(entityType: string, entityId: string | number): Promise<any[]> {
    const result: QueryResult = await query(
      `SELECT al.*, e.name as user_name
       FROM audit_logs al
       LEFT JOIN employees e ON al.user_id = e.id
   ...
   * 批量创建审计日志
   */
  static async createBatch(logs: any[]): Promise<any[]> {
   ...
   * 获取操作类型统计
   */
  static async getActionStats(startDate: string, endDate: string): Promise<any[]> {
    const result: QueryResult = await query(
   ...
   * 获取用户活动统计
   */
  static async getUserStats(startDate: string, endDate: string, limit: number = 10): Promise<any[]> {
    const result: QueryResult = await query(
   ...
   * 获取实体类型统计
   */
  static async getEntityStats(startDate: string, endDate: string): Promise<any[]> {
    const result: QueryResult = await query(
   ...
   * 清理旧日志 (保留策略)
   */
  static async cleanup(beforeDate: string): Promise<number> {
    const result: QueryResult = await query(
