...
class EmployeeModel {
   ...
   * 根据员工编号查询
   */
  static async findByEmployeeNo(employeeNo: string): Promise<any> {
    const result: QueryResult = await query(
   ...
   * 获取当前用户的个人资料 (不包含敏感信息)
   */
  static async getProfile(id: string | number): Promise<any> {
    const result: QueryResult = await query(
   ...
   * 更新个人资料
   */
  static async updateProfile(id: string | number, data: any): Promise<any> {
   ...
   * 根据经理 ID 查询团队成员
   */
  static async findByManager(managerId: string | number): Promise<any[]> {
    const result: QueryResult = await query(
      `SELECT e.*, d.name as department_name, p.title as position_title
       FROM employees e
       LEFT JOIN departments d ON e.department_id = d.id
   ...
   * 根据经理 ID 查询团队成员 (别名方法)
   */
  static async findByManagerId(managerId: string | number): Promise<any[]> {
